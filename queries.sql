...
select count(*) from User, ItemSeller where User.id like ItemSeller.uid and binary User.location like "New York";
-- 103 instead 80
...
select count(*) from (select Item.id, count(ItemCategory.category) as NumberOfCategories from Item, ItemCategory where Item.id=ItemCategory.iid group by Item.id) as CategoryCount where CategoryCount.NumberOfCategories=4;
